...
def run_agent(d,capital,fee,slip,size,threshold):
    cols=["ret1","ret7","ret14","sma20_gap","sma50_gap","rsi","volatility","volume_change"]; z=d.replace([np.inf,-np.inf],np.nan).dropna(subset=cols).copy()
    z=z[z["target"].notna()].copy(); split=max(int(len(z)*.7),1)
